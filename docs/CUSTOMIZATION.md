...
```ts
import { createGuide, themes } from "@yoshihisak/navijs";

createGuide({ id: "x", theme: themes.dark });
// 利用可能: themes.light (既定) / themes.dark / themes.glass / themes.minimal
```

個別 import も可:

```ts
import { darkTheme, glassTheme } from "@yoshihisak/navijs";
```
